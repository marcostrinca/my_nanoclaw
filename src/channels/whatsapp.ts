...
registerChannelAdapter('whatsapp', {
  factory: () => {
      ...
      sock.ev.on('messages.upsert', async ({ messages }) => {
        for (const msg of messages) {
          try {
            ...
            if (fromMe) {
              // MTR: only enforce the self-chat restriction when the assistant
              // has its own number. When it shares the owner's number
              // (ASSISTANT_HAS_OWN_NUMBER=false), the owner's own messages must
              // still wake the agent, so skip the self-chat filter and rely on
              // sentMessageCache alone to drop the bot's own echoes.
    ...
    const adapter: ChannelAdapter = {
      ...
      // MTR: auto-deny unknown WhatsApp contacts instead of sending the owner
      // an approval card for every new DM. Relies on the router.ts auto-deny
      // path keyed on requiresChannelApproval === false.
      requiresChannelApproval: false,
